  ...
  return (
    ...
    <section className="py-5 px-10 my-5" id={`${id}`}  style={{ backgroundColor: bgColor }}>
      <div className="flex justify-between border-b-[1px] border-gray-200">
        ...
        <button className={`text-[${color}] text-xl py-2 px-4 rounded-lg border-[1px] border-[#d5e1e1]`}>
          Show More
        </button>
      </div>

      {/************    Product Cart   **************/}
      <div className={`text-[${color}] flex flex-wrap max-w-[100%] gap-4 py-10 px-2`}>
      {
      products.map((product, index) => (
        <div key={index} style={{ backgroundColor: color }} className="cursor-pointer border-2 border-gray-200 h-[400px] min-w-[200px] max-w-[250px] flex-1 py-3 px-4 mx-auto rounded-md overflow-hidden">
