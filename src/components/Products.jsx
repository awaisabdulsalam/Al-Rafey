...
const Products = ({ products, heading, bgColor, color, id, btnColor }) => {
  ...
  return (
    <>
    <section className="w-full sm:py-5 sm:px-5 md:py-5 md:px-10 my-5" id={`${id}`}  style={{ backgroundColor: bgColor }}>
      ...
      {/************    Product Cart   **************/}
      <div className={`text-[${bgColor}] flex justify-start flex-wrap max-w-[100%] gap-4 py-10 px-2`}>
      {
      products.map((product, index) => (
        <div key={index} className={`sm:min-h-fit relative bg-[#fff] cursor-pointer border-2 border-gray-200 md:h-[400px] min-w-[200px] max-w-[250px] flex-1 py-3 px-3 rounded-md overflow-hidden`}>
